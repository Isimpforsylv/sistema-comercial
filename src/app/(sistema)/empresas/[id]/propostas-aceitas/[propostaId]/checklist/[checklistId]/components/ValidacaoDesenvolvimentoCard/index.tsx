...
interface ValidacaoDesenvolvimentoCardProps {
...
export default function ValidacaoDesenvolvimentoCard({ checklistId, onObservacaoAdded, onEtapaStatusChange, disabled = false }: ValidacaoDesenvolvimentoCardProps) {
  ...
  const fetchEtapa = async () => {
    setLoading(true);
    try {
      const response = await fetch(`/api/checklist/${checklistId}/etapas/Validação de Desenvolvimento`);
      if (response.ok) {
  ...
  const handleDesfinalizar = async () => {
    console.log('handleDesfinalizar chamado - Validação');
    try {
      const response = await fetch(`/api/checklist/${checklistId}/etapas/Validação de Desenvolvimento/finalizar`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
  ...
  if (etapa.finalizada) {
    return (
      <>
        <Accordion>
          <AccordionSummary expandIcon={<ExpandMore />}>
            <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', width: '100%', pr: 2 }}>
              <Typography variant="h6">Validação de Desenvolvimento - Finalizada</Typography>
        ...
        <ObservacaoModal open={obsModalOpen} onClose={() => setObsModalOpen(false)} onSuccess={() => { setObsModalOpen(false); onObservacaoAdded?.(); }} checklistId={checklistId} nometapa="Validação de Desenvolvimento" />
        ...
        <CobrancasModal open={cobrancasModalOpen} onClose={() => setCobrancasModalOpen(false)} onSuccess={() => { setCobrancasModalOpen(false); fetchEtapa(); onObservacaoAdded?.(); }} checklistId={checklistId} nometapa="Validação de Desenvolvimento" dataCobranca={etapa.cobrarem} readonly={true} />
        <ConfirmDialog
  ...
  return (
    <>
      <Card>
        <CardContent>
          <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: 3 }}>
            <Typography variant="h6">Validação de Desenvolvimento</Typography>
            <Box sx={{ display: 'flex', gap: 1 }}>
              <Button startIcon={<NoteAdd />} variant="outlined" size="small" onClick={() => setObsModalOpen(true)} disabled={disabled}>Observação</Button>
              {etapa.cobrarem && (
      ...
      <ObservacaoModal open={obsModalOpen} onClose={() => setObsModalOpen(false)} onSuccess={() => { setObsModalOpen(false); onObservacaoAdded?.(); }} checklistId={checklistId} nometapa="Validação de Desenvolvimento" />
      <HistoricoModal open={historicoModalOpen} onClose={() => setHistoricoModalOpen(false)} historico={historicoTipo === 'dataenvio' ? etapa.historicodataenvio : etapa.historicodataretorno} titulo={historicoTipo === 'dataenvio' ? 'Histórico - Data do Envio' : 'Histórico - Data do Retorno'} />
  <CobrancasModal open={cobrancasModalOpen} onClose={() => setCobrancasModalOpen(false)} onSuccess={() => { setCobrancasModalOpen(false); fetchEtapa(); onObservacaoAdded?.(); }} checklistId={checklistId} nometapa="Validação de Desenvolvimento" dataCobranca={etapa.cobrarem} readonly={disabled} />
      <FinalizarModal open={finalizarModalOpen} onClose={() => setFinalizarModalOpen(false)} onSuccess={() => { setFinalizarModalOpen(false); fetchEtapa(); onObservacaoAdded?.(); onEtapaStatusChange?.(true); }} checklistId={checklistId} nometapa="Validação de Desenvolvimento" dataInicio={etapa.dataenvio} previsaoInicial={etapa.dataretorno} />
    </>
  );
}
