...
export default function PendenciasModal({ open, onClose, onSuccess, melhoriaId, empresaId, propostaId, readonly = false }: PendenciasModalProps) {
  ...
  const handleToggleFinalizada = async (pendenciaId: number, finalizada: boolean) => {
    if (readonly) return;
    try {
      const response = await fetch(`/api/empresas/${empresaId}/propostas-aceitas/${propostaId}/melhoria/${melhoriaId}/pendencias/${pendenciaId}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
  ...
  return (
    ...
    >
      ...
      <DialogContent>
        {/* Removido: Card da Etapa Pendências - agora está na tela principal */}
