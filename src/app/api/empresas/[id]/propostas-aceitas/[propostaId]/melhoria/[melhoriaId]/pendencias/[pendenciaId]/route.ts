...
export async function PUT(
  request: NextRequest,
  context: { params: Promise<{ id?: string; propostaId?: string; melhoriaId?: string; pendenciaId?: string }> }
) {
  try {
    ...
    const pendenciaAtualizada = await (prisma as any).melhoriaPendencia.update({
...
export async function DELETE(
  ...
  try {
    ...
    await (prisma as any).melhoriaPendencia.delete({
      where: { id: pendenciaId },
    });
